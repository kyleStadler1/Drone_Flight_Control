...
void updateAll() {
  ...
  recvr->getSetpoints(&setpoint_roll, &setpoint_pitch, &setpoint_yaw, &setpoint_thr);
  if (recvr->getArm() && !recvr->getFailsafe()) {
    ...
    //Serial.print("R:" + String(imu->getYawVRaw()) + "\tF:" + String(imu->getYawV()) + "\n");
    //Serial.print("Y:" + String(imu->getYawV()) + "\n");
    uint16_t m0, m1, m2, m3;
    pid->updateMotorValues(&m0, &m1, &m2, &m3, false);
