...
//float PID::accumulate(float err, float *I, float deltaS, float K, float *prevErr) {
...
//}
//
//float PID::accumulateV2(float err, float *I, float deltaS, float K, float *prevErr) {
//  *I += err * K;
//  return *I;
//}
//
//int8_t PID::sign(float x) {
//  return (x > 0) - (x < 0); //https://stackoverflow.com/questions/14579920/fast-sign-of-integer-in-c
//}
////get raw pid value per axis
//void PID::getPID(float *roll, float *pitch, float *yaw) {
//  
//  float deltaS = (micros() - dataDate)/1000000.0;
//  float rollErr = setpoint_roll - imu->getRollV();
//  float pitchErr = setpoint_pitch - imu->getPitchV();
//  float yawErr = setpoint_yaw - imu->getYawV();
//  //yawErr = abs(yawErr) > 3 ? yawErr : 0;
//  //Serial.print("YERR:" + String(yawErr) + "\t");
////  float rollPID = (rollErr) * KP_ROLL + accumulate(rollErr, &I_roll, deltaS, KI_ROLL, &prevError_roll) + (-1.0)*imu->getRollA()*KD_ROLL;
////  float pitchPID = (pitchErr) * KP_PITCH + accumulate(pitchErr, &I_pitch, deltaS, KI_PITCH, &prevError_pitch) + (-1.0)*imu->getPitchA()*KD_PITCH;
////  float yawPID = (yawErr) * KP_YAW + accumulate(yawErr, &I_yaw, deltaS, KI_YAW, &prevError_yaw) + (-1.0)*imu->getYawA()*KD_YAW; 
////  
//
//  float rollPID = (rollErr) * KP_ROLL + accumulateV2(rollErr, &I_roll, deltaS, KI_ROLL, &prevError_roll) + (-1.0)*(prevError_roll - rollErr)*KD_ROLL;
//  float pitchPID = (pitchErr) * KP_PITCH + accumulateV2(pitchErr, &I_pitch, deltaS, KI_PITCH, &prevError_pitch) + (-1.0)*(prevError_pitch - pitchErr)*KD_PITCH;
//  float yawPID = (yawErr) * KP_YAW + accumulateV2(yawErr, &I_yaw, deltaS, KI_YAW, &prevError_yaw) + (-1.0)*(prevError_yaw - yawErr)*KD_YAW; 
//  
//
...
//
//
//void PID::getMotorValues(uint16_t *m0, uint16_t *m1, uint16_t *m2, uint16_t *m3, float roll, float pitch, float yaw) {
...
//}
//
//void PID::updateSetpoints(float r, float p, float y, uint16_t thr) {
//  setpoint_roll = r;
//  setpoint_pitch = p;
//  setpoint_yaw = y;
//  if (thr < THR_IDLE) {
